...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FCombatPressedSignature, bool, bCombatState);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FForwardBackwardPressedSignature, float, fForwardBackwardValue);
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category = "ALS|Input")
	FCombatPressedSignature CombatPressedDelegate;

	UPROPERTY(BlueprintAssignable, Category = "ALS|Input")
	FForwardBackwardPressedSignature ForwardBackwardPressedDelegate;
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Camera System")
	void GetCameraParameters(float& TPFOVOut, float& FPFOVOut) const;
...
protected:
	...
	UPROPERTY(Category="ALS|Camera", EditAnywhere, meta=(ClampMin="1.0", ClampMax="100.0"))
