...
ECollisionChannel AALSBaseCharacter::GetThirdPersonTraceParams(FVector& TraceOrigin, float& Radius)
{
...
void AALSBaseCharacter::GetCameraParameters(float& TPFOVOut, float& FPFOVOut) const
{
...
void AALSBaseCharacter::ForwardMovementAction_Implementation(float Value)
{
	...
	if (ForwardBackwardPressedDelegate.IsBound())
	{
		ForwardBackwardPressedDelegate.Broadcast(Value);
	}
	...
	} else if (MovementState == EALSMovementState::Climbing){
		
		FVector Direction;
		// Get the climbing surface normal
		FVector ClimbSurfaceNormal = MyCharacterMovementComponent->GetClimbSurfaceNormal();
		
		Direction = FVector::CrossProduct(ClimbSurfaceNormal, -GetActorRightVector());
		
		// Prevent rotation flickering
		FRotator SurfaceRotation = CharacterMesh->GetComponentRotation();
		SurfaceRotation.Pitch = 0.0f; // Remove any Pitch rotation
...
void AALSBaseCharacter::CombatAction_Implementation(bool bValue)
{
	if (CombatPressedDelegate.IsBound())
	{
		CombatPressedDelegate.Broadcast(bValue);
	}
}
...
FCollisionQueryParams AALSBaseCharacter::GetIgnoreCharacterParams() const
{
	FCollisionQueryParams Params;
