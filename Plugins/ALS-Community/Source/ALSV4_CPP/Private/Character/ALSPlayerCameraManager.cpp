...
const FName NAME_CameraBehaviorAnim(TEXT("CameraBehaviorAnim"));
...
bool AALSPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
	...
	float TPFOV = 90.0f;
	float FPFOV = 90.0f;
	ControlledCharacter->GetCameraParameters(TPFOV, FPFOV);
	...
	TargetCameraRotation = UKismetMathLibrary::RLerp(InterpResult, DebugViewRotation,
	                                                 GetCameraBehaviorParam(NAME_Override_Debug), true);
	...
	if(ControlledCharacter->GetMovementState() != EALSMovementState::Climbing){
		ViewOffset = DebugViewOffset;
	}
	...
	TargetCameraLocation = UKismetMathLibrary::VLerp(
		...
		UKismetMathLibrary::GetForwardVector(TargetCameraRotation) * GetCameraBehaviorParam(
			NAME_CameraOffset_X) +
		 UKismetMathLibrary::GetRightVector(TargetCameraRotation) * ViewOffset.X +
		UKismetMathLibrary::GetRightVector(TargetCameraRotation) * GetCameraBehaviorParam(NAME_CameraOffset_Y) + UKismetMathLibrary::GetForwardVector(TargetCameraRotation) * ViewOffset.Y
		+
		UKismetMathLibrary::GetUpVector(TargetCameraRotation) * GetCameraBehaviorParam(NAME_CameraOffset_Z)
		+	UKismetMathLibrary::GetUpVector(TargetCameraRotation) * ViewOffset.Z,
	...
	const FCollisionShape SphereCollisionShape = FCollisionShape::MakeBox(FVector(TraceRadius, TraceRadius, TraceRadius));
	...
	if(HitResult.IsValidBlockingHit()){
		LastHitResult = TargetCameraLocation + (HitResult.Location - HitResult.TraceEnd);
	} else {
		LastHitResult = TargetCameraLocation;
	}
	
	if(CurrentCameraLocation.IsZero()){
		CurrentCameraLocation = TargetCameraLocation;
	}
	
	float LerpingSpeed = DeltaTime / CameraCatchupTime;
	LerpingSpeed = FMath::Clamp(LerpingSpeed, 0.0f, 1.0f);  // Clamp the value between 0 and 1.
	
	// Lerp calculation without using Kismet.
	CurrentCameraLocation = CurrentCameraLocation + (LastHitResult - CurrentCameraLocation) * LerpingSpeed;
	...
	FTransform TargetCameraTransform(TargetCameraRotation, CurrentCameraLocation, FVector::OneVector);
	...
	const FTransform& TargetTransform = UKismetMathLibrary::TLerp(MixedTransform,
	                                                              FTransform(DebugViewRotation, CurrentCameraLocation,
														FVector::OneVector),
													GetCameraBehaviorParam(
													NAME_Override_Debug));
