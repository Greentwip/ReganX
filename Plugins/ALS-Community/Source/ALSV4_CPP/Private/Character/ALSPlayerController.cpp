...
AALSPlayerController::AALSPlayerController(){
	PlayerCameraManager = CreateDefaultSubobject<AALSPlayerCameraManager>(TEXT("PlayerCameraManager"));
}
...
void AALSPlayerController::CombatAction(const FInputActionValue& Value)
{
	if (PossessedCharacter)
	{
		PossessedCharacter->CombatAction(Value.Get<bool>());
	}
}
