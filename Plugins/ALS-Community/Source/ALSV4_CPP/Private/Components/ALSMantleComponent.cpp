...
const FName NAME_ClimbTimeline(TEXT("ClimbTimeline"));
const FName NAME_ClimbEnd(TEXT("ClimbEnd"));
const FName NAME_ClimbUpdate(TEXT("ClimbUpdate"));
...
UALSMantleComponent::UALSMantleComponent()
{
	...
	ClimbTimeline = CreateDefaultSubobject<UTimelineComponent>(NAME_ClimbTimeline);
}
...
void UALSMantleComponent::BeginPlay()
{
	...
	if (GetOwner())
	{
		...
		if (OwnerCharacter)
		{
			...
			FOnTimelineFloat ClimbTimelineUpdated;
			FOnTimelineEvent ClimbTimelineFinished;

			ClimbTimelineUpdated.BindUFunction(this, NAME_ClimbUpdate);
			ClimbTimelineFinished.BindUFunction(this, NAME_ClimbEnd);

			ClimbTimeline->SetLooping(true);
			ClimbTimeline->SetTimelineLengthMode(TL_TimelineLength);
			ClimbTimeline->AddInterpFloat(MantleTimelineCurve, ClimbTimelineUpdated);

			ClimbTimeline->SetTimelineFinishedFunc(ClimbTimelineFinished);

			OwnerCharacter->ForwardBackwardPressedDelegate.AddUniqueDynamic(this, &UALSMantleComponent::OnOwnerForwardBackwardInput);
                                        ...
                                        FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (OwnerCharacter)
	{
		if (OwnerCharacter->GetMovementState() == EALSMovementState::InAir && !bClimbing)
		{
		...
		else if (OwnerCharacter->GetMovementState() == EALSMovementState::Grounded && OwnerCharacter->HasMovementInput())
		{
			fClimbTimer += DeltaTime;
			
			if (fClimbTimer >= TimeToClimb)
			{
				fClimbTimer = TimeToClimb;
		...
		else if(OwnerCharacter->GetMovementState() == EALSMovementState::Climbing && OwnerCharacter->HasMovementInput() && bClimbing){
			if(MantleCheck(FallingTraceSettings, EDrawDebugTrace::Type::ForOneFrame)){
				bClimbing = false;
				fClimbTimer = 0.0f;
			}
		}
		else if(OwnerCharacter->GetMovementState() != EALSMovementState::Climbing)
		{
			bClimbing = false;
			fClimbTimer = 0.0f;

			if(ClimbTimeline->IsPlaying()){
				ClimbTimeline->Stop();
				OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_Stop(0, MantleParams.AnimMontage);
...
bool UALSMantleComponent::TryClimb()
{
	FHitResult SurfHit;
	FVector CapLoc = OwnerMovement->UpdatedComponent->GetComponentLocation();
	GetWorld()->LineTraceSingleByProfile(SurfHit, CapLoc, CapLoc + OwnerMovement->UpdatedComponent->GetForwardVector() * OwnerMovement->ClimbReachDistance, "BlockAll", OwnerCharacter->GetIgnoreCharacterParams());
	
	// Trace upward to check for obstacles above
	FVector TraceStartUp = CapLoc;
	FVector TraceEndUp = TraceStartUp + FVector(0.0f, 0.0f, OwnerMovement->CapHH());
	FHitResult SurfHitUp;
	const FCollisionShape CapsuleCollisionShape = FCollisionShape::MakeCapsule(OwnerMovement->CapR(), OwnerMovement->CapHH());
	const bool bHitUp = GetWorld()->SweepSingleByProfile(SurfHitUp, TraceStartUp, TraceEndUp, FQuat::Identity, MantleObjectDetectionProfile,			  CapsuleCollisionShape, OwnerCharacter->GetIgnoreCharacterParams());

	if (!SurfHit.IsValidBlockingHit() || SurfHitUp.IsValidBlockingHit()) return false;
                                      ...
                                      EALSMantleType MantleType)
{
	if (OwnerCharacter == nullptr || !IsValid(MantleLedgeWS.Component) || !IsValid(MantleTimeline) ||
		!IsValid(ClimbTimeline))
	{
		return;
	}
	
	ClimbTimeline->Stop();
	OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_Stop(1, MantleParams.AnimMontage);
...
void UALSMantleComponent::ClimbStart()
{
	if (OwnerCharacter == nullptr || !IsValid(ClimbTimeline))
	{
		return;
	}
		
	// Step 1: Get the Mantle Asset and use it to set the new Mantle Params.
	const FALSMantleAsset MantleAsset = GetMantleAsset(EALSMantleType::Climbing, OwnerCharacter->GetOverlayState());
	check(MantleAsset.PositionCorrectionCurve)
	
	MantleParams.AnimMontage = MantleAsset.AnimMontage;
	MantleParams.PlayRate = MantleAsset.HighPlayRate;
	ClimbTimeline->PlayFromStart();
	
	// Step 7: Play the Anim Montage if valid.
	if (MantleParams.AnimMontage && OwnerCharacter->GetMesh()->GetAnimInstance())
	{
		ClimbTimeline->SetTimelineLength(MantleAsset.AnimMontage->GetPlayLength());
		
		ClimbTimeline->SetPlayRate(MantleParams.PlayRate);

		OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_Play(MantleParams.AnimMontage, MantleParams.PlayRate,
																   EMontagePlayReturnType::MontageLength,
																   MantleParams.StartingPosition, true);
		
		OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_Pause(MantleParams.AnimMontage);
	}
	
	ePreviousRotationMode = OwnerCharacter->GetRotationMode();
	OwnerCharacter->SetRotationMode(EALSRotationMode::LookingDirection);
}
...
void UALSMantleComponent::ClimbUpdate(float BlendIn){
	
}

void UALSMantleComponent::ClimbEnd(){
	
}

void UALSMantleComponent::OnOwnerForwardBackwardInput(float Value)
{
	if(bClimbing)
	{
		float NewPosition = OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_GetPosition(MantleParams.AnimMontage) + (Value / 60.0f) * ClimbTimeline->GetTimelineLength() * MantleParams.PlayRate;
		
		
		
		// If you want to loop, reset to the start
		if (NewPosition > ClimbTimeline->GetTimelineLength())
		{
			NewPosition = 0;
		}
		
		if(NewPosition < 0){
			NewPosition = ClimbTimeline->GetTimelineLength();
		}
				
		if (Value == 0)
		{
			ClimbTimeline->SetPlayRate(0.0f);
		...
		else
		{
			if (MantleParams.AnimMontage && OwnerCharacter->GetMesh()->GetAnimInstance())
			{
				// Set the position and play rate
				OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_SetPosition(MantleParams.AnimMontage, NewPosition);
				OwnerCharacter->GetMesh()->GetAnimInstance()->Montage_SetPlayRate(MantleParams.AnimMontage, MantleParams.PlayRate);
			}
		}
	}
}
...
void UALSMantleComponent::OnOwnerJumpInput()
{
	...
	} else if (OwnerCharacter && OwnerCharacter->GetMovementAction() == EALSMovementAction::None)
	{
...
void UALSMantleComponent::MantleStop(){
	OwnerCharacter->SetRotationMode(ePreviousRotationMode);
}
